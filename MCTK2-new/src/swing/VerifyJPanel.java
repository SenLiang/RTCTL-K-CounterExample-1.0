...
public class VerifyJPanel implements ActionListener {
    ...
    final String atltips = "Please input a ATL*SPEC...";
    SMVModule main;//避免多次读取main
    ...
    public VerifyJPanel(IndexJFrame indexJFrame) {
        ...
        JLabel rtctlBar = new JLabel("   ATLstarSPEC   ");
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == copy) {
            ...
            if (atltips.equalsIgnoreCase(specific) || specific.equals("") || specific.trim().startsWith("--"))
                ...
                GRun("ATL*SPEC ".concat(specific), false);
            else
                GRun("ATL*SPEC ".concat(specific) + ";", false);
            ...
            if (atltips.equalsIgnoreCase(specific) || specific.equals("") || specific.trim().startsWith("--"))
                ...
                GRun("ATL*SPEC ".concat(specific), true);
            else
                GRun("ATL*SPEC ".concat(specific) + ";", true);
    ...
    protected void NewATLSpec(String property) {
        JPanel specp = new JPanel(new FlowLayout(FlowLayout.CENTER));
        JLabel atlBar = new JLabel("        ATLstarSPEC          ");
        JButton specmB = new JButton(minIcon);
        specmB.setContentAreaFilled(false);
        specmB.setBorderPainted(false);
        JButton specaB = new JButton(plusIcon);
        ...
        String tips = "Please input a ATL*SPEC...";
        JTextArea specT = new JTextArea();// 文本窗格
        specT.setPreferredSize(new Dimension(500, 45));
        specT.setLineWrap(true);
    ...
    public void ReadSMVSpec() {
        ...
        if (name.equals("")) {
            ...
            try {
                ...
                Statistic getStat=new Statistic();
                insertDocument(ctext, "\nInitial occupation\nNumber of BDD nodes:" + getStat.beginBDD +
                        "\nNumber of BDD variables:"+getStat.beginVar, Color.GRAY, 1);
            }
    ...
    public boolean ExtractSpec() {
        ...
        for (int i = 0; i < all_specs.length; i++) {
            ...
            } else if (all_specs[i].startsWith("ATL*SPEC")) {
                if (atltips.equals(atlText.getText()))
                    atlText.setText(all_specs[i].replaceAll("ATL\\*SPEC", "").toString());
                else
                    NewATLSpec(all_specs[i].replaceAll("ATL\\*SPEC", "").toString());
            }
        }
        return true;
    }
    ...
    public void GRun(String parse, Boolean isgraph) {
        ...
        for (int i = 0; i < all_specs.length; i++) {
            ...
            } else if (all_specs[i].getLanguage() == InternalSpecLanguage.LTL || all_specs[i].getLanguage() == InternalSpecLanguage.ATLs) {
                ATLsK_ModelCheckAlg algorithm = new ATLsK_ModelCheckAlg(main, all_specs[i]);
                if (isgraph) {//带图的反例
                    algorithm.SetShowGraph(true);
                    runner = new AlgRunnerThread(algorithm);
                    runner.runSequential();
