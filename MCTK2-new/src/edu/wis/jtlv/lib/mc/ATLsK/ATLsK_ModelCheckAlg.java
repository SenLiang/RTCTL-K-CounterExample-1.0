...
public class ATLsK_ModelCheckAlg extends ModelCheckAlgI {
    ...
    //spec <-> Tester.x
    private HashMap<Spec, BDD> STMap = new HashMap<Spec, BDD>();
    private static Vector<SMVModule> SaveTester = new Vector<SMVModule>();
    private static final Vector<BDD> old_ini_restrictions = null;
...
//		this.tester = user_tester;
    ...
    public BDD sat(Spec spec,                // an ATL*K specification
        ...
        if (op == Operator.AND) {
            ...
            BDD c3 = c1.and(c2);
            STMap.put(child[0], c1);
            STMap.put(child[1], c2);
            STMap.put(spec, c3);
            return c3;
        }
        if (op == Operator.OR) {
...
//                    new ValueConsStrStmt(tester, new String[]{w + ""}))).eval_stmt().toBDD();
    ...
    public BDD satAA(Spec spec) throws ModelCheckException, SMVParseException, ModuleException, ModelCheckAlgException, SpecException {
        ...
        STMap.put(child[0], negC1.not());
        SaveTester.add(negC1Tester);
    ...
    public BDD satCAN_ENFORCE(Spec spec) throws ModelCheckException, SMVParseException, ModuleException, ModelCheckAlgException, SpecException {
        ...
        STMap.put(child[child.length - 1], negC1.not());
        if (!tester_isEmpty(negC1Tester)) {
            design.syncComposition(negC1Tester);
    ...
    public BDD satCANNOT_AVOID(Spec spec) throws ModelCheckException, SMVParseException, ModuleException, ModelCheckAlgException, SpecException {
        ...
        STMap.put(child[child.length - 1], c1);
        if (!tester_isEmpty(c1Tester)) {
            design.syncComposition(c1Tester);
...
*/
    ...
    public AlgResultI preAlgorithm() throws AlgExceptionI, SMVParseException, ModelCheckException, ModuleException {
        ...
        for (SMVModule tester : SaveTester) {
            ModuleBDDField[] TesterVar = tester.getAllFields();
            for (ModuleBDDField var : TesterVar) {
                all_couples.remove(var);
            }
            design.decompose(tester);
        }
        SaveTester.removeAllElements();//reset SaveTester
        return null;
    }
    ...
    public AlgResultI doAlgorithm() throws AlgExceptionI, ModelCheckException, ModuleException, SMVParseException, SpecException {
        //System.out.println("model checking ATL*K property: " + property);
        //GExample.insertDocument(ctext,  "\n model checking ATL*K property: " + property, Color.BLACK,1);
        if (!property.isStateSpec()) {
            chkProp = NNF(new SpecExp(Operator.EE,
                    new SpecExp(Operator.NOT, property))); // newp = E !property
        } else { // the property is a state formula
        ...
        System.out.println("After NNF get the formula--" + chkProp);
        //GExample.insertDocument(ctext,  "\n After NNF get the formula--" + chkProp, Color.BLACK,1);
        visibleVars = this.getRelevantVars(getDesign(), chkProp);
        // now chkProp is a state property
        SMVModule chkPropTester = null;
        chkBdd = sat(chkProp, chkPropTester); // after executing sat function, newpTester must be null
    ...
    public boolean Witness(
        ...
        System.out.println("Witness------" + spec+"---"+(spec instanceof SpecBDD));
        if ((spec instanceof SpecBDD)|| ((SpecExp) spec).getOperator() == Operator.AA || (!spec.hasTemporalOperators() && !spec.hasPathOperators())) {
            ...
            Node n = G.addStateNode(++createdPathNumber, 0, fromState, null, 0); // create the first state 1.0 of G
            ...
            if (op == Operator.EE) {
                ...
                Node n = G.addStateNode(pathNo, stateNo, fromState, null, 0); // create the first state 1.0 of G1、G2
                n.setAttribute("ui.class", "initialState");
                TreeLikeModel(child[0], G, pathNo, stateNo);
            } else if (op == Operator.AND) {
                Witness(child[1], initState, G, pathNo, 0);//1
                Witness(child[0], initState, G, ++pathNo, 0);
            } else if (op == Operator.OR) {
                BDD c1 = STMap.get(child[1]);
                if (!initState.and(c1).isZero())
...
//        }
        ...
        System.out.println(spec.isStateSpec()+"+++++++++++++"+spec.hasPathOperators());
        if (spec.isStateSpec()) {// state formula
            if (op == Operator.EE || op == Operator.AA)// E(X f), E(f U g), E(f U a..b g),
                TreeLikeModel(child[0], G, pathNo, stateNo); //E(E(E Xf)) nesting path quantifiers
            else if (op == Operator.KNOW) {//E(dc1 KNOW dc2.said)
                SMVModule negC1Tester = new SMVModule("Tester" + (++tester_id));
                SpecExp newSpec = new SpecExp(Operator.NOT, child[1]);
                BDD negC1 = sat(newSpec, negC1Tester);
                STMap.put(newSpec, negC1);
                SpecAgentIdentifier agentId = (SpecAgentIdentifier) child[0];
                witnessNKnow(agentId.toString(), newSpec, G, pathNo, stateNo);
            } else if (op == Operator.NKNOW) {//E(dc1 NKNOW dc2.said)
                SpecAgentIdentifier agentId = (SpecAgentIdentifier) child[0];
                witnessNKnow(agentId.toString(), child[1], G, pathNo, stateNo);
            }
        } else if (!spec.isStateSpec() && spec.hasPathOperators()) { // fTester is empty
            if (op == Operator.AND) {     //(g &(E f))
                TreeLikeModel(child[0], G, pathNo, stateNo);
                ...
                if (!fromState.and(c1).isZero())
                    ...
                    TreeLikeModel(child[0], G, pathNo, stateNo);
            } else if (op == Operator.NEXT) {//(X(E f))
                 witnessX(spec, G, pathNo, stateNo);
            } else if (op == Operator.UNTIL)//(g U(E f))
                witnessU(spec, G, pathNo, stateNo);
            //Operator.RELEASES、Operator.B_UNTIL
        } else if (!spec.isStateSpec() && !spec.hasPathOperators()) {// Xf, fUg
...
//            }
//            if (op == Operator.UNTIL||op == Operator.RELEASES)
//            {
//                System.out.println("child0- "+child[0]+"  Tester- "+STMap.get(child[0]));
//                System.out.println("child1- "+child[1]+"  Tester- "+STMap.get(child[1]));
//            }
...
//        }
        ...
        String stateID = pathNo + "." + stateNo;
        if (subspec instanceof SpecBDD) {
            G.addNodeSatSpec(stateID, subspec, false);
            return true;
        }
        BDD fromState = G.getNodeBDD(stateID);
        ...
        if (op == Operator.EE || op == Operator.AA)
            ...
            BDD X0 = STMap.get(child[0]), X1 = STMap.get(child[1]);
            int j = stateNo;
            while (true) {
            ...
            BDD X0 = STMap.get(child[0]), X2 = STMap.get(child[2]);
            SpecRange range = (SpecRange) child[1];
            int a = range.getFrom();
            int b = range.getTo();
            int j = 0, k = stateNo;
    ...
    public boolean witnessU(
        ...
        BDD f = STMap.get(child[0]);
        BDD g = STMap.get(child[1]);
    ...
    public boolean witnessNKnow(
        ...
        if (fromState == null || fromState.isZero()) return false;
        BDD specStates = null;
        specStates = STMap.get(spec).and(getDesign().feasible());
