...
public class SetGraphThread implements Runnable, ViewerListener, ActionListener {
    ...
    public void buttonPushed(String id) {
        ...
        } else if (graph instanceof GraphExplainATLsK) {
            str = ((GraphExplainATLsK) graph).getNodeSatSpec(id);
            if (str != null && !str.equals(""))
                insertDocument(nodeConsole, "\n" + "[satisfies " + str + "]", Color.BLACK, 1);
            insertDocument(nodeConsole, "\n" + ((GraphExplainATLsK) graph).getNodeStateDetails(id), Color.BLACK, 1);
            try {
                ((GraphExplainATLsK) graph).getChecker().explainOneGraphNode(((GraphExplainATLsK) graph), id);
            } catch (ModelCheckAlgException e) {
                e.printStackTrace();
